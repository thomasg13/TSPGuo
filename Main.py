...
def draw_path_segment(i):
    global window  # Window must be declared global
    if i < len(cities) - 1:
...
def solve_tsp(algorithm):
    ...
    start_time = time.perf_counter()  # Make timer more precise, more decimal values
...
def path_length(path): # Calculate path length, helper method
    if len(path) < 2:
        return 0
